...
# ---------------- CONFIG (fill these) ----------------
TOKEN = "MTQ5NzczNjU2NTQ0OTgxODExMg.GgPVra.wJ4a20uDM9clF4DKFEeSOvX9aQfnsfajF-ZRxU"
GUILD_ID = 1474581259262103765  # your guild id as int

MATCH_TIMES_CHANNEL_ID = 1474595501860065300
ASSIGNMENTS_CHANNEL_ID = 1497776514878668840
TRANSACTIONS_CHANNEL_ID = 1474586935086551282
MATCH_SCORES_CHANNEL_ID = 1474595714498564106
# ----------------------------------------------------

# Global role IDs (right‑click role > Copy ID with Developer Mode on)
GLOBAL_CAPTAIN_ROLE_ID = 1476021423268040764   # captain role ID
GLOBAL_COCAPTAIN_ROLE_ID = 1476021467673268234 # co‑captain role ID
GLOBAL_PLAYER_ROLE_ID = 1476021533548744816    # player role ID
...
def get_team_roles(guild: discord.Guild, team_name: str):
    # Only per-team ROLE, captain/co-captain/player are now global
...
class AcceptView(discord.ui.View):
    ...
    async def _handle_accept(self, interaction: discord.Interaction, target_team: str):
        ...
        if self.team1 in self.accepted_for and self.team2 in self.accepted_for:
            ...
            try:
                # 1) Post clean info message in match-times (no buttons)
                if match_channel:
                    match_message = await match_channel.send(base_text)

                # 2) Post claim message in assignments with buttons
                if assign_channel:
...
class PromoteCoCaptainView(discord.ui.View):
    def __init__(self, actor: discord.Member, team_name: str, candidates: List[discord.Member]):
        ...
        select.callback = self.promote_callback  # type: ignore
    ...
    async def promote_callback(self, interaction: discord.Interaction):
        ...
        if tx_ch:
            await tx_ch.send(
                f"{target.mention} has been promoted to co-captain in {self.team_name} by {self.actor.mention}"
            )
...
class DemoteCoCaptainView(discord.ui.View):
    def __init__(self, actor: discord.Member, team_name: str, cocaps: List[discord.Member]):
        ...
        select.callback = self.demote_callback  # type: ignore
        self.add_item(select)
    ...
    async def demote_callback(self, interaction: discord.Interaction):
        ...
        if tx_ch:
            await tx_ch.send(
                f"{target.mention} has been demoted from co-captain in {self.team_name} by {self.actor.mention}"
            )
...
class KickMemberView(discord.ui.View):
    def __init__(self, actor: discord.Member, team_name: str, members: List[discord.Member]):
        ...
        select.callback = self.kick_callback  # type: ignore
        self.add_item(select)
    ...
    async def kick_callback(self, interaction: discord.Interaction):
        ...
        roles_to_remove = []
        for r in (team_role, player_role, cocap_role):
...
async def code(interaction: discord.Interaction, team1: discord.Role, team2: discord.Role):
    ...
    # Generate random code: AGT + 4 digits
    code_value = f"AGT{random.randint(1000, 9999)}"
...
async def add_team(interaction: discord.Interaction, member: discord.Member, team_name: str):
    ...
    # Use global player role + team role
    _, _, global_player_role = get_global_team_roles(guild)
    roles_to_add = [team_role]
    if global_player_role and global_player_role not in member.roles:
...
async def captain_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title=f"AGT Captain Panel – {team_name}",
        description=desc,
    ...
    embed.set_footer(text="AGT Transactions Bot")
...
async def co_captain_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title=f"AGT Co-Captain Panel – {team_name}",
        description=desc,
...
async def addscrim(interaction: discord.Interaction, team1: discord.Role, team2: discord.Role):
    ...
    await channel.send(
        ...
        f"# Welcome to AGT seeding #.\n"
        f"🗓️ You guys will have 3 days to schedule\n"
...
@bot.tree.command(
    guild=discord.Object(id=GUILD_ID),
    name="info",
    description="Show information about the AGT Transactions Bot commands"
)
async def info(interaction: discord.Interaction):
    ...
    embed.set_footer(text="AGT Transactions Bot • created by banner")
